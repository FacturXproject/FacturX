...
class InvoiceViewIntegrationTest extends AbstractIntegrationTest {
	...
	private long uploadXml(Cookie session, long orgId, byte[] xmlBytes) throws Exception {
	...
	void uploadedCiiXmlCanBeDisplayedAsInvoiceView() throws Exception {
		...
		long documentId = uploadXml(session, orgId, validCiiXmlBytes());
		...
		mockMvc.perform(
				...
				.andExpect(jsonPath("$.lines[0].description").value("Consulting service"))
	...
	void malformedXmlReturnsCleanError() throws Exception {
		...
		long documentId = uploadXml(session, orgId, malformedXml);

		mockMvc.perform(
						get("/api/documents/" + documentId + "/invoice-view")
								.cookie(session))
				.andExpect(status().isUnprocessableEntity())
				.andExpect(jsonPath("$.error").value("INVALID_INVOICE_XML"));
	}
